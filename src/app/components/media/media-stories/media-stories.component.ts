...
export class MediaStoriesComponent implements OnInit {
  ...
  loadStories(index: number): void {
    // Set stories user index for navigation.
    this.feedIndex = index;
    // Set clicked user index as the starting point.
    ...
    if (!this.feedStories[this.feedIndex].items) {
      ...
      this.getStories(index);
    } else { this.openStories(); }
  }

  async getStories(index: number): Promise<void> {
    let stories = [];
  ...
  async storiesNextSkip(): Promise<void> {
    ...
    if ((this.feedIndex == (this.feedStories.length - 1) ||
      ...
      // Get user stories if the selected user has none loaded yet.
      if (!this.feedStories[this.feedIndex + 1].items) { await this.getStories(this.feedIndex + 1); }
      this.storiesIndex = 0;
      this.feedIndex++;
      this.storyIndex();
      this.storySeen();
